...
async function loadActivities(page = 1) {
    ...
    result.data.forEach(act => {
        ...
        const startDate = App.formatDateTime(act.start_date);
        const endDate = act.end_date ? App.formatDateTime(act.end_date) : '-';
        const eventDate = act.event_date ? App.formatDateTime(act.event_date) : App.formatDate(act.start_date);
            ...
            <div class="card h-100 shadow-sm activity-card" onclick="location.href='./?page=activity-detail&id=${act.id}'" style="cursor:pointer">
                ...
                <div class="card-body">
                    ...
                    <p class="card-text small text-muted">
                        <i class="bi bi-calendar-event me-1"></i><strong>วันจัดกิจกรรม:</strong> ${eventDate}<br>
                        <i class="bi bi-calendar-plus me-1"></i><strong>วันเริ่มต้น:</strong> ${startDate}<br>
                        <i class="bi bi-calendar-check me-1"></i><strong>วันสิ้นสุด:</strong> ${endDate}<br>
                        <i class="bi bi-door-open me-1"></i><strong>การรับสมัคร:</strong> ${statusBadge.replace(/<[^>]+>/g, '')}<br>
